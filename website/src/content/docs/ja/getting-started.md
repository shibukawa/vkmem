...
変わるのは、その下にある土台です。vkmemにはスレッドも`fork`もなく、すべてをメモリ上に置きます。バックグラウンド保存とAOFの書き換えは失敗し、I/Oスレッドは起動せず、レプリケーション、クラスタモード、TLSは使えません。詳しくは[互換性](../compatibility/)にまとめています。
...
| Node.js、Java、その他 | 各言語のパッケージ経由の`vkmem-server` | テストランナーの子プロセスとして、ループバックのポートとUnixソケットで待ち受ける |
...
- [Goガイド](../go/)、[Node.jsガイド](../node/)、[Javaガイド](../java/)
